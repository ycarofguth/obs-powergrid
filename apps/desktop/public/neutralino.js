    ...
    ;((a = new WebSocket(`ws://${o}:${window.NL_PORT}?connectToken=${e}`)),
      ...
      a.addEventListener('message', (e) => {
        ...
        t.id && t.id in s
          ...
          : t.event &&
            ('openedFile' == t.event &&
              'dataBinary' == t?.data?.action &&
              (t.data.data = r(t.data.data)),
            n(t.event, t.data))
      }),
  ...
  function l(e, t) {
    return new Promise((n, r) => {
      ...
      const i = '10000000-1000-4000-8000-100000000000'.replace(/[018]/g, (e) =>
        ...
        u = g()
      ;((s[i] = { resolve: n, reject: r }),
        a.send(JSON.stringify({ id: i, method: e, data: t, accessToken: u })))
    })
  }
  async function f(e) {
    for (; e.length > 0; ) {
